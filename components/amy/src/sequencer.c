...
void sequencer_init(int max_sequencer_tags) {
    ...
    // LOCAL EDIT (S3-Amysynth): OOM guard - disable the sequencer wholesale
    // instead of writing through a NULL table (sequencer_add_wire and
    // sequencer_check_and_fill already treat sequences == NULL as "not
    // initialized", so every downstream path degrades to a no-op).
    if (sequences == NULL) {
        amy_oom("sequencer_init");
        max_sequences = 0;
        first_active = -1;
        return;
    }
