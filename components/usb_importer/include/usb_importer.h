 ...
 *                             fmt = "txt" (amysong text) or "mid" (SMF)
 *                             bars = converter bars for "mid", ignored for "txt"
 *                             -> ACK\n (or ERR:<reason>\n)
 *                             then exactly <len> raw bytes,
 *                             -> OK:saved to slot <n>\n (or ERR:<reason>\n)
 *
 * The request is parsed and the payload captured on the CDC pump task; the
 * actual song_import_apply / midi_amysong_convert runs on the sequencer's
 * single-applier ui task via usb_import_service(), exactly like the WiFi
 * importer. Call usb_import_service() from the same task that calls
 * wifi_import_service().
 */
