...
static void dispatch_button_event(my_button_id_t button_id, button_event_t event)
{
    /* MY_BUTTON_TRANSPORT (optional dedicated play/stop pad): tap toggles
     * playback in every view. Like the menu key this is a global gesture, so
     * editors and the arp/drone guards below do not consume it first. The
     * long-press play/stop on MY_BUTTON_0 stays untouched. */
    if (button_id == MY_BUTTON_TRANSPORT) {
...
/* Mirror the transport to external gear: as the sync master we send realtime
 * Start/Stop so slaves follow the play/pause key (the 0xF8 clock stream flows
 * continuously from AMY's sequencer either way). No-op unless AMY is in
 * AMY_MIDI_SYNC_SEND mode with a UART MIDI out configured. */
static void midi_clock_transport_cb(bool playing)
{
    if (playing) {
        midi_clock_out_start();
    } else {
        midi_clock_out_stop();
    }
}
...
void app_main(void)
{
    ...
    /* As the sync master, AMY's sequencer streams 0xF8/0xFA/0xFC to the MIDI
     * UART; the mode is a no-op when no UART MIDI is configured. */
#if CONFIG_AMYSYNTH_MIDI_CLOCK_TX_GPIO >= 0
    ...
    static const midi_sink_t s_live_sink = {
        ...
        .cc         = live_play_cc,
        .pitch_bend = live_play_pitch_bend,
        .clock_tick  = midi_clock_slave_tick,
        .clock_start = midi_clock_slave_start,
        .clock_stop  = midi_clock_slave_stop,
    };
